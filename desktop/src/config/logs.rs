...
use crate::config::properties::LoggingProperties;
...
fn filter(logging: &LoggingProperties) -> String {
    let directives = logging
        .level
        .iter()
        .filter(|(target, _)| target.as_str() != ROOT)
...
mod tests {
    ...
    fn targets_become_filter_directives() {
        let directives = filter(&logging(&[("root", "info"), ("econbox", "TRACE")]));

        assert!(directives.starts_with(bevy::log::DEFAULT_FILTER));
        assert!(directives.ends_with("econbox=trace"));
